...
function isASIVANTALeak(text) {
  return /ASIVANTA|pet|dog|cat|veterinary|grooming|walking/i.test(text);
}
...
export default async function handler(req, res) {
  ...
  const safeAnswer =
    bridgeAnswer && !isASIVANTALeak(bridgeAnswer)
      ? bridgeAnswer
      : fallback.answer;
  ...
  return res.status(200).json({
    ...
    fallback: !bridgeAnswer || isASIVANTALeak(bridgeAnswer),
  });
}
